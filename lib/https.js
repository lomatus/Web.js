	...
	exports.server = https.createServer(function (req, res) {
		 ...
		 */
			...
			res.redirect = function (url) {
				switch (url) {
					case 'home':
						res.send('<!DOCTYPE html><meta http-equiv="refresh" content="0; url=http://' + req.headers.host + '" />');
						break;
					case 'back':
						res.send('<!DOCTYPE html><script type="text/javascript">history.go(-1);</script>');
						break;
					case 'refresh':
						res.send('<!DOCTYPE html><meta http-equiv="refresh" content="0" />');
						break;
					default:
						res.send('<!DOCTYPE html><meta http-equiv="refresh" content="0; url=' + url + '" />');
				}
				return this;
			}
		/*
